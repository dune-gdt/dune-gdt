...
static inline const Common::Configuration& default_eigen_solver_options()
{
  static const Common::Configuration opts = []() {
    Common::Configuration ret = common_eigen_solver_options(/*compute_eigenvectors=*/true);
...
/// \brief Assembles the options of an eigen solver of the given type, an empty type selects the preferred one.
static inline Common::Configuration eigen_solver_options_for_type(const std::vector<std::string>& available_types,
                                                                  const std::string& type)
{
  const std::string actual_type = type.empty() ? available_types[0] : type;
  ensure_eigen_solver_type(actual_type, available_types);
  return solver_options_with_type(default_eigen_solver_options(), actual_type);
} // ... eigen_solver_options_for_type(...)
