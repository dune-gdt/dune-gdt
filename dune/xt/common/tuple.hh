...
// Authors:
//   Felix Schindler (2012, 2014 - 2017, 2020)
//   René Fritze     (2010 - 2013, 2015 - 2016, 2018 - 2020)
//   Stefan Girke    (2012)
//   Tobias Leibner  (2014, 2016 - 2018, 2020)

/// \file
/// \brief Template metaprogramming helpers for working with std::tuple and type lists.
...
/// \brief Clamps the tuple index no_ to a valid index of tuple type t_ (returns 0 if t_ has fewer elements).
#define TMAX(t_, no_) (std::tuple_size<t_>::value >= (no_ + 1) ? no_ : 0)
/// \brief Extracts the nested type s_ of the no_-th element of tuple type t_ (clamped via TMAX).
#define TELE(t_, s_, no_) typename std::tuple_element<TMAX(t_, no_), t_>::type::s_

/// \brief Builds a std::tuple from up to nine types, keeping only the first Size of them.
...
//! tag type marking the end of the recursive type-combination generated by Combine
...
//! a compile-time sequence of indices (a pre-C++14 std::index_sequence replacement)
template <size_t... Is>
...
//! builds an index_sequence<0, 1, ..., N-1> with O(log(N)) template recursion depth
template <size_t N>
struct make_index_sequence
{
...
//! like std::tuple_element, but with O(log(N)) template instantiation depth
...
//! empty tag type used to mark the (missing) tail of a single-element type list
struct tuple_null_type
{};
//! empty tag type used as a generic null/placeholder type
struct null_type
{};

//! splits a tuple/template_tuple type list into its head type and tail list
...
//! a tuple-like type list that may also hold (not-yet-instantiated) class templates wrapped via tplwrap
