...
#
# By default vcpkg installs both variants side by side: release into <triplet>/lib and debug into
# <triplet>/debug/lib. A Debug consumer build (CMAKE_BUILD_TYPE=Debug) then straddles both trees -- per-config
# imported targets (e.g. gmpxx via dune/mpfr) resolve to debug/lib while module-mode finds (e.g. find_package(LAPACK))
# resolve to the release lib. Because libgmpxx.so.4 and liblapack.so.3 carry the same SONAME in *both* directories,
# CMake cannot order the RPATH (debug/lib must precede lib for gmpxx, lib must precede debug/lib for lapack) and emits
# the harmless-but-noisy "Cannot generate a safe runtime search path ... cycle in the constraint graph" warning for
# every target.
#
# Pinning the dependencies to a single configuration removes the debug/lib tree entirely, so there is nothing to
# mis-order and the warnings disappear. Release is chosen over debug because we never step through dependency
# internals; this also roughly halves the dependency build time. Our own dune/dune-gdt libraries are still built with
# the consumer's CMAKE_BUILD_TYPE, and Debug<->Release is ABI-compatible here (no _GLIBCXX_DEBUG), which the current
# build already relies on by linking the release lapack into the Debug build.
