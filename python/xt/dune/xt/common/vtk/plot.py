...
if config.HAVE_K3D:
    ...
    def _transform_to_k3d(timestep, poly_data, color_attribute_name):
        ...
        indices = numpy_support.vtk_to_numpy(poly_data.GetPolys().GetData()).reshape(
            -1, 4
        )[:, 1:4]
    ...
    def plot(
        ...
        if not interactive:
            ...
            c_dist = (
                np.sin((90 - fov_angle) * np.pi / 180)
                * absx
                / (2 * np.sin(fov_angle * np.pi / 180))
            )
        ...
        if size > 1:
            ...
            slider = IntSlider(
                min=0, max=size - 1, step=1, value=0, description="Timestep:"
            )
