...
for mod_name in (
    ...
    "_functions_generic_function",
    "_functions_generic_grid_function",
    "_functions_gradient",
...
if config.HAVE_K3D:
    ...
    def visualize_function(functions, grid, interactive=False, subsampling=False):
        ...
        def visualize_single(func, filename):
            try:
                ...
                GridFunction(grid, func).visualize(
                    grid, filename=filename, subsampling=subsampling
                )
    ...
    def visualize_function_on_dd_grid(function, dd_grid, subdomains=None):
        ...
        tmpfile = tempfile.NamedTemporaryFile(
            mode="wb", delete=False, suffix=".vtu"
        ).name
        ...
        try:
            plot(
                prestring + tmpfile[5:-4] + ".pvtu", color_attribute_name=function.name
            )
        except AttributeError:
            if 1 < len(subdomains) < dd_grid.num_subdomains:
                # TODO: fix this error
                print(
                    "For 1 < len(subdomains) < num_subdomains, the generated .pvtu file is known "
