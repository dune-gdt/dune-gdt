...
#
#   * CustomMetadataHook supplies the build-time-dynamic `dependencies`. mpi4py is required only when the C++ bindings
#     were built against MPI; this is recorded as __have_mpi__ in the CMake-generated dune/xt/_version.py. We read that
#     file directly instead of importing dune.xt, which would pull in metis and the compiled extension modules;
#     _version.py contains only plain assignments, so executing it in an empty namespace is safe.
...
class CustomMetadataHook(MetadataHookInterface):
    def update(self, metadata):
        ...
        exec(
            (Path(self.root) / "dune" / "xt" / "_version.py").read_text(
                encoding="utf-8"
            ),
            _version,
        )

        dependencies = ["ipython", "numpy", "scipy"]
        if _version["__have_mpi__"]:
            dependencies.append("mpi4py")
